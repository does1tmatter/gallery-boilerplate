...
<template>
  <RouterLink v-for="(item, i) in data" :key="i" :to="`${url}${getTokenId(item.name)}`" class="relative uppercase tracking-tighter hover:scale-[1.03] transition-all ">
      <div :data-aos="route.name === 'collection' ? 'fade-in' : ''" class="rounded-xl overflow-hidden shadow-xl shadow-black/[.15] dark:shadow-purple-200/[.15]">
